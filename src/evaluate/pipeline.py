...
def _build_pipeline(model: str = "ollama/llama3.1:8b", max_tokens: int = 500):
    ...
    try:
        ...
        print(f"  Cannot build live pipeline: {e}")
        return None
...
def run_evaluation(
    ...
    if domain:
        ...
        print(f"  Filtered to domain '{domain}': {len(pairs)} pairs")
    else:
        pairs = qa_set.get_all()
        print(f"  Full test set: {len(pairs)} pairs")
    ...
    if limit and limit < len(pairs):
        ...
        while len(sampled) < limit:
            for d in sorted(by_domain):
                ...
                break
        pairs = sampled[:limit]
        print(f"  Sampled {len(pairs)} pairs across {len(by_domain)} domains (limit={limit})")
    ...
    for i, pair in enumerate(pairs):
        ...
        print(f"  [{i+1}/{len(pairs)}] {qa_id} ({domain_tag}/{difficulty}): {q[:80]}...")
    ...
    print(f"  Hallucination report: {out_path / 'hallucination_report.json'}")

    # Summary
    print(f"\n  === EVALUATION SUMMARY ===")
    print(f"  Questions: {len(pairs)} | Answered: {len(ok_results)}")
