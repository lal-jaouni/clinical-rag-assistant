...
class RAGASEvaluator:
    ...
    def faithfulness(answer: str, context: list[str]) -> float:
        ...
        Blends unigram overlap (word-level grounding) with bigram overlap
        (phrase-level grounding). Unigrams capture paraphrased content that
        LLMs produce; bigrams capture exact phrasing fidelity.
        """
        ...
        unigram_overlap = sum(1 for w in answer_words if w in context_word_set) / len(answer_words)

        ans_bg = _ngrams(answer_words, 2)
        ctx_bg = _ngrams(context_words, 2)
        bigram_overlap = len(ans_bg & ctx_bg) / max(len(ans_bg), 1)
