    ...
    def load(self, **kwargs: Any) -> list[dict[str, Any]]:
        ...
        for entry in self.documents:
            ...
            try:
                ...
                print(f"  [skip] {doc_id}: download failed: {type(e).__name__}: {e}")
                continue

            try:
                text = self._extract_text(pdf_path)
            except Exception as e:
                print(f"  [skip] {doc_id}: text extraction failed: {type(e).__name__}: {e}")
                continue

            if not text.strip():
                print(f"  [skip] {doc_id}: extracted text is empty")
                continue
            ...
            print(f"  [ok]   {doc_id}: {len(text)} chars, {len(sections)} sections")
