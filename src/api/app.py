...
async def lifespan(app: FastAPI):
    ...
    logger.info(f"Starting clinical RAG API (model={model})...")
    try:
        _pipeline, _session = _build_pipeline(model)
        logger.info("Pipeline ready.")
    except Exception as e:
        logger.warning(f"WARNING: Pipeline init failed: {e}")
        logger.warning("API will start but /query will return 503.")
    yield
    if _session:
        _session.close()
